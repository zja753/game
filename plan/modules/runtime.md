...
now(): number; // 毫秒,统一时钟(暂停时不走,见 §5.3 FrameClock)
...
viewportSize(): { width: number; height: number }; // 视口尺寸(像素),Camera 用来对齐 Scene.camera.pos
collision: {
...
返回当前画布视口的像素尺寸(跟着 Excalibur `screen.resolution` / `screen.viewport` 走,resize 时自动更新)。

**为什么需要它?**

- Camera 模块每帧把摄像机位置写入 `Scene.camera.pos`,而 Excalibur 的摄像机坐标原点是"屏幕左上角相对世界的位置",需要先用视口大小算出"玩家在屏幕中央时,世界坐标应该 = 玩家位置 - 视口一半"。

**屏幕外裁剪的入口**:**不**走 `RuntimePort.viewportSize()`。Combat / Enemy 如果要做"超出视口就跳过 AI / 跳过绘制",应该调 `CameraPort.isOnScreen(pos)`(见 [`./camera.md` §2](./camera.md#2-对外-port))—— 那个函数已经把 `viewportSize` + `mapBounds.clampRange` 的几何算好,直接拿 `boolean` 即可,免得每个模块自己再算一遍"摄像机边界"还和 Camera 的 `computeCameraPos` 不一致。

**使用场景**:

- Camera 模块每帧:`const { width, height } = runtime.viewportSize(); const half = Vector.half(width, height); ...`

> 教学点:这个方法**只是 Excalibur 视口信息的只读视图**,**不**做 viewport 调整 —— resize 处理由 `§5.1 EngineFactory` 在初始化时挂监听器负责,模块不要自己去监听 `window.resize`。
...
| Camera      | `viewportSize()`(每帧读)、`onTick`(推摄像机跟随)                                    | `spawnActor` 摄像机关联节点(可选,通常 Camera 不挂 Actor) |
