...
> 顶层路线见 [`../modular-roadmap.md`](../modular-roadmap.md)。本文件是 Combat 模块的自留地:Port / 事件 / 内部子模块拆分 / 验收点都在这里。
...
- **Demo 页** `/demo/combat`:Mock 3 个静止"敌人"在 (100,0) / (0,200) / (-200,0),点 `tryFire`,断言:
  - 第一个被命中的是 (100,0)(最近)
  - `projectile:hit` 事件 1 次,`damage=10`
  - 远到 400px 外的"敌人"按 5 次 `tryFire` 0 次发射(`TargetSelector` 不消耗节流)
- **vitest**:
  - `HitResolver` 对 1 发弹命中血剩 5 的敌人 → `isKill=true`
  - 命中血剩 50 的敌人 → `isKill=false`
  - `WeaponRegistry` 切换 `swapWeapon` 后 `currentWeapon()` 正确
  - `damageDealt()` / `kills()` 累加正确
