...
- **它利用别人的能力**:Runtime(spawn 投射物) / MapObstacle(投射物撞墙) / Enemy(读列表选目标 + 写伤害)。
- **它不做什么**:敌人行为、玩家移动、经验值。
...
- **它利用别人的能力**:Runtime(spawn 敌人) / Player(接触伤害) / Progression(拿关卡配置)。
- **它不做什么**:投射物、武器、关卡。
...
- **能力**:维护摄像机世界坐标(玩家跟随 + 地图边缘 hard clamp,无 smoothing / 无过渡动画)、视口大小、当前 clamp 范围;每帧把摄像机位置写入 Excalibur `Scene.camera`。**完全复刻土豆兄弟原版手感**——玩家走到地图边缘时摄像机停在边缘,玩家相对摄像机位置偏移;玩家没到边缘时摄像机永远把玩家钉在屏幕正中。
- **利用它做什么**:HudUi 订阅 `camera:moved` 做小地图 / 屏幕边缘提示;Combat / Enemy 在做屏幕外裁剪时通过 `ctx.camera` 拿视口范围(可选优化,首版不强制)。
